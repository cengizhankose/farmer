...
export class RiskCalculator {
    ...
    calculateRiskScore(data) {
        ...
        const estimatedParticipants = this.estimateParticipants(data.tvlUsd, data.volume24h || 0, data.protocol);
        ...
        const totalScore = Math.round(liquidityScore.score * this.config.weights.liquidity +
    ...
    calculateStabilityRisk(input) {
        ...
        if (input.tvl_30.length < 7)
            confidence *= 0.6;
        if (input.tvl_90.length < 30)
            confidence *= 0.8;
        return {
            score: totalScore,
            factors,
    ...
    calculateYieldSustainabilityRisk(input) {
        const factors = [];
        let totalScore = 0;
        let confidence = 1.0;
        ...
        // Adjust confidence based on data availability
        if (input.apr_30.length < 7)
            confidence *= 0.7;
        if (input.apr_90.length < 30)
            confidence *= 0.8;
        return {
            score: totalScore,
            factors,
    ...
    calculateConcentrationRisk(input) {
        const factors = [];
        let totalScore = 0;
    ...
    // Helper methods
    estimateParticipants(tvlUsd, volume24h, protocol) {
    ...
    calculateVolatility(values) {
        if (values.length < 2)
            return 0;
        const returns = values.slice(1).map((val, i) => values[i] > 0 ? (val - values[i]) / values[i] : 0);
    ...
    calculateConfidence(confidences) {
