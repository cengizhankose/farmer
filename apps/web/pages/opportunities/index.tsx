    ...
    <div className="mx-auto max-w-6xl px-4 py-12">
      <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-zinc-600">Normalized APR/APY, risk labels, and freshness. Multi-chain preview clearly disabled.</p>
        </div>
      ...
      <div className="mt-6 grid grid-cols-1 gap-5 md:grid-cols-2">
        {filtered.map((o) => {
          ...
          return (
            <Card key={o.id} className={`relative overflow-hidden border-white/40 bg-white/60 backdrop-blur-2xl ${disabled ? 'opacity-60' : ''}`}>
                ...
                {disabled ? (
                  <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger>
                        <span>
                          <Button disabled variant="secondary" className="w-full">View details</Button>
                        </span>
                      </TooltipTrigger>
                      <TooltipContent>Coming soon on {CHAINS.find(c=>c.id===o.chain)?.label}</TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
                ) : (
                  <Button className="w-full bg-emerald-600 text-white hover:bg-emerald-700" onClick={() => router.push(`/opportunities/${o.id}`)}>
                    View details
                  </Button>
                )}
              </div>
      ...
      <div className="mt-10 flex items-start gap-2 text-xs text-zinc-600">
        <Info size={14} />
        <p>NFA. Data is mocked for demo. Actual adapter-backed data and router (B) integration coming next.</p>
      </div>
    </div>
  );
}
