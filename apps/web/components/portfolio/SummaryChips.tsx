...
export const SummaryChips: React.FC<{ rows: RedirectEntry[] }>
  = ({ rows }) => {
    ...
    const Chip = ({ label, value }: { label: string; value: string }) => (
      <div className={`rounded-md bg-[${colors.zinc[100]}] px-3 py-2 text-sm`}>
    ...
    return (
      <div className="mt-6 grid grid-cols-1 gap-3 md:grid-cols-3">
        <Chip label="Toplam anapara" value={formatUSD(total)} />
        <Chip label="Ort. APR" value={`${avgAPR.toFixed(1)}%`} />
        <Chip label="Tahmini toplam getiri" value={formatUSD(estTotal)} />
      </div>
    );
  };
